...
const CACHE_PREFIX = "stitch-app-shell-"
const CACHE_NAME = `stitch-app-shell-${BUILD_ID}`
...
async function networkFirst(request) {
  ...
  try {
    ...
    if (request.mode === "navigate") {
      return (await cache.match(request)) ?? (await cache.match(APP_ROOT_URL)) ?? (await cache.match(APP_INDEX_URL)) ?? response
    }

    return response
  } catch {
    return (await cache.match(request)) ?? (await cache.match(APP_ROOT_URL)) ?? (await cache.match(APP_INDEX_URL)) ?? Response.error()
  }
}
